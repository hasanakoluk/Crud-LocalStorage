...
function Read(){
    ...
    if(getUserList != null && getUserList.length === 0){
        nameListDOM.innerHTML = "Hiç kullanıcı yok"
    }else{
...
function Edit(item){
 const editUsers = JSON.parse(localStorage.getItem("userList"))
 nameListDOM.innerHTML = ""
 for (let i = 0; i < editUsers.length; i++) {
  ...
  <input type="text" id="newName" placeholder="${editUsers[i]}"/>
    ...
    <div class="buttons">
      ...
      <button class="danger" onclick="Read()">
        <i class="fas fa-trash"></i>
        Cancel
      </button>
    </div>
